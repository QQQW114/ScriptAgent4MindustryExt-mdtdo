...
- 新增指令一律要进对应分区并分类（用户口径，2026-09-12）：本轮新增的 `/quickops` 放在**管理指令**分区首位；
  `/votestartban`（禁止发起投票）、`/voteunban`（解除禁止投票）、`/votelist`（禁止投票名单）放在管理分区
  禁封相关条目附近；`/vote sc` 的条目在**投票指令**分区，用法行已更新为 `[文字] [秒数]`。
...
- `/vote sc [文字] [秒数]`：SuperChat 中屏醒目留言（不发起实际投票）；1级信任以上、冷却2分钟。
  秒数为**屏幕中央停留时间**，缺省 3 秒、**最多 5 秒**（超出按 5 秒处理）；为保证到点收起，
  服务端用 `setHudText` 定时刷新并在到时 `hideHudText`（原版 `announce` 的时长由客户端决定、服务端无法指定）。
- `/votestartban <玩家id|三位id> [理由]`（别名 `禁投票`/`禁止投票`/`voteban`）：**3++/4级**禁止指定玩家发起投票；
  也可在**玩家信息菜单**（双击玩家）里一键"禁止ta发起投票"/"解除禁止发起投票"。
  只拦截"发起投票"，不影响该玩家参与别人发起的投票；状态经脚本 config 持久化，跨重启保留。
- `/voteunban <玩家id|三位id>`（别名 `解禁投票`/`解除禁止投票`）：解除上述限制。
- `/votelist`（别名 `禁投票名单`/`votebanlist`）：查看当前被禁止发起投票的玩家与理由。
- `/quickops`（别名 `快捷操作`/`协管菜单`/`qops`/`opmenu`）：**3++/4级快捷操作菜单**。
  列出当前**全部投票项**（含各脚本新增的项，自动同步），点击即**直接执行**，无需发起投票与他人同意；
  执行走 `RootCommands.handleInput`，与玩家手打 `/vote <子项>` 完全同路径，权限照常校验；
  需要参数的项（usage 带 `<>`/`[]`）不在菜单内空参数执行，而是提示改用指令补参数。
