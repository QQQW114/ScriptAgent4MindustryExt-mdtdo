...
- 已在开发服按上述分阶段方案迁移到 SA 3.4.0，并修到 `sa fail` 无输出。
- 数据库必须采用 SA 3.4 `coreLibrary/db` + `Services` 结构；旧 kts `ServiceRegistry` 在隔离类加载器下会稳定复现 `No Provider`。
- 取消附身后的核心机问题没有使用全量 `sendWorldAndAssets` 修复，而是新增仅同步核心单位与 Player 的定向实体快照兼容层，避免再次触发资产/世界重载。
- 生产切换前仍需真实客户端完成取消附身、进服、换图、观战和数据库功能回归。
