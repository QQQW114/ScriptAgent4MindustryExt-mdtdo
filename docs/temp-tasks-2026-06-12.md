...
# 当前任务：崩溃自动重启与 v159 ZIP CP（2026-07-22）

- [x] Windows/Linux 启动脚本默认在 JVM 异常退出后自动重启。
- [x] 连续崩溃使用指数退避，正常退出与终止信号不自动拉起。
- [x] 外部 CP 扫描新增 `.json5` / `.zip`。
- [x] 支持 v159 ZIP 的 patches/content/bundles/sprites/sounds/music。
- [x] ZIP 路径、文件数、解压大小、压缩比、PNG尺寸和资产冲突检查。
- [x] Patch/Content运行态错误和缓存缺失校验。
- [x] 加载/热重载/卸载失败恢复完整 Data Assets。
- [x] Patch、Content、Bundle、Sprite 冒烟包加载和卸载验证。
- [ ] 生产服继续观察包含真实音乐、音效和大型贴图的 CP 包，以及多人同步期间的上行情况。
# 当前任务：点歌冷却与封禁管理（2026-07-22）

- [x] 点歌同意率不再统计未表态玩家。
- [x] 投票结果显示已表态同意率与未表态人数。
- [x] `PlayerBanStore` 支持列出所有未到期封禁。
- [x] 玩家/账号封禁和 IP 封禁合并为 `/banlist` 菜单。
- [x] 菜单显示原因、剩余时长、关联标识和操作人。
- [x] 玩家封禁与 IP 封禁均可从详情页快速解除。
- [x] 冷启动及控制台实际列表查询验证通过。

# 当前任务：直接强制观战权限收紧（2026-07-22）

- [x] 直接强制未登录游客观战由2级提高到3级。
- [x] 游客判断改为实时会话认证状态，不使用资料显示等级0代替。
- [x] 非管理员成功直接操作后按主体UID冷却120秒。
- [x] 记录强制观战来源与操作者UID，非管理员仅可撤回自己的面板直接操作。
- [x] 管理员全局解除、投票解除和3+原有权限保持兼容。
- [x] 冷启动脚本编译验证通过：156脚本、加载152、启用148、出错0。

# 当前任务：3++ 插件协管（2026-07-22）

- [x] 新增3++等级、排序、名称、解析别名与3级赞踩额度映射。
- [x] 3++/4仅人工任命，自动晋升系统不产生也不降级。
- [x] 新增独立 `@pluginAdmin` 白名单权限组，不继承 `@admin`。
- [x] 3++使用与4级相同的管理图标，但不能使用 `/suffixmark`。
- [x] 3+、3++、4级的处罚目标边界改为中央分层判断。
- [x] 授予投票保护、有限封禁/IP封禁、换图/结束、最近玩家、内容安全和点歌应急白名单。
- [x] 3++封禁上限默认7天，解封仅限自己记录；高风险地图操作加入二次确认/冷却/审计。
- [x] 同步信任系统、帮助菜单、安全风控与维护文档。
- [x] 完成最终冷启动编译验证与目标日志复核：156脚本、加载152、启用148、出错0；无编译/未解析引用错误。

# 当前任务：收紧3+直接限制目标（2026-07-22）

- [x] 3+直接强制观战和禁建的目标上限调整为低于3级，仅包含0/1/2级。
- [x] 3++仍可处理低于3++的目标，4级保留全局处理；账号/IP封禁等独立能力不随本项误收紧。
- [x] 完成冷启动编译验证：156脚本、加载152、启用148、出错0。
- [x] 提交本轮改动。

# 当前任务：统一3+即时限制边界（2026-07-22）

- [x] 3+直接强制观战、禁言、禁建统一只能处理0/1/2级玩家。
- [x] 3++仍可处理低于3++的目标，4级保留全局处理。
- [x] 禁言/禁建入口及实际执行函数增加实时权限复核，避免菜单状态过期造成越权。
- [x] 完成冷启动编译验证：156脚本、加载152、启用148、出错0。

# 当前任务：惊鸿 ZIP 同名生成贴图兼容（2026-07-22）

- [x] 对照 Mindustry v159.7 源码确认 `generated/**` 贴图的原生优先级。
- [x] 仅放行同一 ZIP 内一张 generated 与一张普通同名图的合法组合。
- [x] 保持跨包、地图/服务端资产、第三张同名图和多 generated 冲突硬拒绝。
- [x] 将具体消歧路径写入加载警告和服务端日志。
- [x] 使用 `惊鸿.zip` 完成加载、热重载、卸载和 generated 路径验证。
- [x] 使用临时冲突包验证跨包覆盖与三重同名资产仍被拒绝。
- [x] 冷启动编译验证通过：156脚本、加载152、启用148、出错0。

# 当前任务：核心机/附身单位闪回复查（2026-07-22）

- [x] 确认 B480 后的核心机兼容脚本会在首次进服、世界确认和核心单位变化时叠加 0/1/3/8 秒重试。
- [x] 确认首份可靠实体快照会在 TCP 拥塞后成为延迟旧状态，与新 UDP 快照无跨通道顺序保证；重复 `PlayerSpawn` 也会在客户端执行 `player.set(core)` 导致可见回弹。
- [x] 删除可靠实体快照、额外 `PlayerSpawn` 和无条件四轮重试；世界确认后已有核心单位则只补 UDP 快照，单位为空时才尝试生成，最多两份 `Unit -> Player` 快照。
- [x] 玩家附身非核心单位时立即作废旧 generation；每次发包前复核连接状态、generation 和单位对象身份。
- [x] 复核 `syncThrottle.kts`：它确实降低包含单位移动/控制关系的全局实体快照频率，可使高丢包时的插值更粗；但它不拦截、重放或删除单位。
- [x] 复核 `serverPressureActions.kts`：玩家当前附身单位、`spawnedByCore`、`TimedKillc`、`BuildingTetherc` 和不可击杀单位均排除在压力清理外。
- [x] 隔离目录先从空缓存验证 `coreUnitRespawnCompat` 编译/加载/启用；最终代码再次触发该脚本重新编译，避开当前运行服务端的 RPC 端口后完整汇总为156/152/148/0。
- [ ] 真实客户端完成高上行、首次进服、附身/取消附身与内部世界重同步回归。
