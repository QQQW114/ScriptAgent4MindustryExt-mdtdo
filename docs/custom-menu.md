...
> **2026-09-19 补充：跨菜单"返回上一页"（`MenuNav`）**
>
> 问题：帮助菜单（`coreMindustry/menu.kts`）等入口用 `RootCommands.handleInput` 打开其它系统菜单
> （`/posts`、`/wiki`、`/achievements`），这些系统自己的"返回/关闭"只会关掉自己，回不到入口那页。
>
> 做法：新增模块库 `coreMindustry/lib/menuNav.kt`，提供 `MenuNav.push(player, label) { 重开上一页 }` /
> `peek` / `take` / `clear`：
> - 入口菜单在跳转**之前** push（帮助菜单已接入 `/posts`、`/wiki`、`/shop`、`/skill` 与各条目列表页，
>   回归动作是"重开同一页/同一分页"）；
> - 目标菜单用 `MenuNav.peek` 决定是否渲染 `返回` 按钮，点击时 `take` 取出并执行动作；
> - 每玩家**单层**（后进覆盖）、**TTL 3 分钟**，`push/peek` 时顺带清理过期项，不注册监听器，内存 O(在线玩家)；
> - 没有来源菜单（玩家自己敲指令进来）时 `peek` 为空 → 保持原来的单个"关闭"，行为不变。
>
...
>    旧聊天菜单（连带它自己的"返回"）残留在屏幕上，形成"两个不同样式的返回"。
>    修法：**把跳转命令丢到 `launch(Dispatchers.game)` 执行**，让原回调立刻返回。
