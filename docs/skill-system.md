...
- 3级技能：资历3级及以上玩家可查看/使用；默认不受普通 `@noSkills` 限制，但仍受 PVP 限制；纯净模式（投票、管理员或地图 `[@pure]` 标签）会额外添加 `@pureNoLevel3Skills`，此时 3级技能也被禁用。
- 特殊/商店技能：分类对所有玩家可见；具体技能只有已购买且满足资历/认可/消耗等使用条件时显示；默认受 PVP 与 `@noSkills` 限制，少数理财/天气类技能会显式标注不受限制。
- 管理员技能：资历4级玩家可查看/使用；信任4级或已登录的 Mindustry 原生 admin 默认视为资历4级；不受 PVP、`@noSkills`、MDC 消耗和冷却限制。信任3++属于有限协管，即使资历被手动设为4也不能使用管理员技能。
...
| `basicdefense` | 初级预制防线 | 0 | 一局一次 | 释放前要求以脚下为锚点的 4x4 范围内没有已有方块；放置范围压到敌方出生点及出怪保护半径时会拒绝释放；以玩家脚下作为中心 2x2 的左下格，按炮台→供电/电池→墙体顺序建造 4x4 铜墙防线；中心 2x2 包含 `arc`、2个 `solar-panel`、1个 `battery` |
| `extinguish` | 灭火 | 0 | 120秒 | 从玩家中心向外散射原版海啸水子弹，并清理周围10格火焰兜底（**纯 tile 实现**：逐格 `Fires.has/extinguish/remove` + `Fx.fireRemove`，不遍历实体；见下"灭火技能口径"） |
...
| `coreZone` | 核心区 | 2 MDC | 120秒 | 生成 3x3 核心区地板（放置范围压到敌方出生点及出怪保护半径时拒绝释放） |
...
本轮三级技能的正数MDC消耗统一按原值 `×1.5` 调整；MDC 只能使用整数，因此遇到 `.5` 时向上取整（例如 `15 -> 23`）。`nuke` 单独调整为 `60 MDC`，原本为 `0` 的 `firetruck` 仍保持 `0`。

| code | 显示名 | 消耗 | 冷却 | 效果 |
|---|---|---:|---:|---|
| `blitz` | 骇人空袭 | 15 MDC | 一局一次 | 在玩家位置快速召唤三波自爆苍穹，每波间隔0.2秒，目标点限制在光标方向约45格内；短时间翻倍队伍坠毁伤害 |
| `antiarmor` | 反装甲炮击 | 23 MDC | 300秒 | 在玩家鼠标位置标记半径20格炮击区域，持续30秒；区域内出现敌方单位时锁定最高血量目标，每1.5秒造成目标最大生命10%+800伤害 |
| `pddCut` | 拼夕夕砍一刀 | 15 MDC | 一局一次 | 全场单位当前血量减少 90% |
| `disaster` | 天灾 | 15 MDC | 一局一次 | 随机天气 120 秒，期间每 10 秒削减全场单位 5% 当前血量 |
| `redLightGreenLight` | 123木头人 | 15 MDC | 一局一次 | 停止所有单位，并随机击杀 10 个单位 |
| `gaokao` | 参加高考 | 30 MDC | 每日一次 | 随机 0-750 分；400 分以下按分数扣 MDC，400 分及以上按分数给 MDC；700+授予 `[gold]高考状元` |
| `firetruck` | 消防车 | 0 | 300秒 | 6秒内每秒从玩家中心向外散射原版海啸水子弹，并清理周围10格火焰兜底（**纯 tile 实现**，同 `extinguish`） |
| `omg` | omg | 23 MDC | 0 | omg |
| `standarddefense` | 标准预制防线 | 23 MDC | 一局一次 | 释放前要求以脚下为锚点的 6x6 范围内没有已有方块；放置范围压到敌方出生点及出怪保护半径时会拒绝释放；以玩家位置为蓝瑟左下坐标，按炮台→供电/电池→墙体顺序建造塑钢墙蓝瑟防线；蓝瑟周边除底部两格电池外均为太阳能板 |
| `missileVolley` | 导弹齐射 | 15 MDC | 一局一次 | 在玩家位置附近召唤 10 个 `scathe-missile-surge`；参考 `/gather` 拒绝非 targetable 单位，E 星 `evoke` / `incite` / `emanate` 三种核心机不可使用，且拒绝检查发生在扣除 MDC 之前 |
| `supplyitem` | 物资补给 | 15 MDC | 0 | 打开当前内容物资菜单或输入物资英文/本地名；为本队核心添加指定物资 x100；PVP 禁用 |
| `randommaga` | 随机maga | 30 MDC | 一局一次 | 召唤一只 `mega`，并直接塞入随机有效建筑方块作为载荷；随机池排除地形/地板/矿物覆盖层、无分类/隐藏/调试/无法构造为载荷的无效方块；仍可突破载荷上限 |
| `nuke` | 核弹打击 | 60 MDC | 一局一次 | 释放时广播光标坐标，5秒后在玩家光标位置触发钍反应堆爆炸效果与范围伤害；抵达提示为“核打击已经抵达！” |
| `refreshskills` | 刷新技能 | 150 MDC | 300秒 | 清除自己当前所有技能冷却；本技能会在释放后重新进入300秒冷却 |
| `tietie` | 贴贴 | 150 MDC | 0 | 指向在线玩家发送贴贴请求；接受后双方静止并传送贴贴，随后生成随机T1-T4单位；拒绝/超时则在释放者位置生成随机T1-T3单位 |
...
| `floodon` / `floodoff` | 开启/关闭洪水脚本 | 启用/停用 `mapScript/tags/flood`；关闭时先确认脚本已停用再删除 `@flood/@floodV2`，失败则保留/回滚标签 |
| `lordon` / `lordoff` | 开启/关闭Lord脚本 | 启用/停用 `mapScript/14668`并返回真实失败原因；该脚本是整图脚本，不保证任意地图适配 |
...
| `puremodeon` | 开启本局纯净模式 | 立即为当前局启用 `@noSkills` 与 `@pureNoLevel3Skills`，同时禁用普通/2级与3级技能 |
| `puremodeoff` | 关闭本局纯净模式 | 关闭当前局纯净模式，并精确恢复开启前两个技能标签的原状态，不误删地图原有限制 |

`floodon/floodoff`、`lordon/lordoff` 不单独实现脚本生命周期管理；它们复用 `wayzer/map/funRuleModes.kts` 中与 `/loadmapscript`、`/unloadmapscript` 相同的加载/关闭逻辑。关闭时会显式创建 ScriptAgent 事务、检查目标最终状态，并避免连带停用原本已启用的公共脚本（例如 `coreMindustry/contentsTweaker`）；管理员技能界面会显示底层返回的真实错误，不再只提示“查看日志”。
...
- `/loadmapscript <id或路径>`：尝试加载 `scripts/mapScript/<id>.kts`。
  ...
  - `/unloadmapscript <id或路径>` 使用同一套安全关闭逻辑，关闭时不扫描磁盘，只关闭目标地图脚本，并恢复误被连带停用的常驻脚本。
...
- `/vote pure`：投票通过后立即为当前这局开启纯净模式，添加 `@noSkills` 与 `@pureNoLevel3Skills`；换图后不自动延续。
- `/vote pure status`：查看当前局纯净模式和两个技能限制标签状态。
- `/vote pureoff`：投票通过后立即关闭当前局纯净模式，并恢复开启前的标签状态。

## 当前商店技能

| 商品ID | 技能code | 显示名 | 购买要求 | 使用限制 |
|---|---|---|---|---|
| `1` | `radar` | 雷达 | 60 MDC，最低资历1级 | 使用4 MDC，冷却600秒；**解除战争迷雾 300 秒**（不再要求地图原本开着雾：记录释放前的 fog 值，到点按原值恢复；连续释放以最后一次为准） |
...
| `5` | `corezone4` | 核心区4x4 | 120 MDC，最低资历1级 | 使用6 MDC，冷却1200秒，生成4x4核心区（同样受出生点保护限制） |
...
`extinguish` / `firetruck` 的火焰处理**只按 tile 实现**（`clearNearbyFires()`：以玩家中心 10 格半径逐格
`Fires.has(tile.x, tile.y)` → `Fires.extinguish(tile, 1f)` + `Fires.remove(tile)` + `Fx.fireRemove`），
**不再遍历 `Groups.all` 找 `Fire` 实体、也不使用已删除的 `Groups.fire` 分组**。

- 技能主机制不变：散射原版海啸水弹（`emitTsunamiWaterScatter`）本身就是原版灭火路径，tile 清理只是
  "周围 10 格"的即时兜底；`/firetruck` 的 6 秒持续散射同理。
- 代价固定：每次释放最多查询 21×21 格，与场上实体总数无关（旧实现是"每次释放全量扫实体"）。
- 火焰**蔓延**的开关由性能保护负责（`state.rules.fire = false`，L1 措施，压力恢复时还原）；
  灭火技能只处理**已经烧起来**的局部火焰，两者职责不重叠。
- 相关：`wayzer/reGrief/limitFire.kts`（按火焰数量自动关闭火焰）已于 2026-09-13 整脚本删除，
  详见 [性能保护](performance-guard.md#火焰处理2026-09-13移除全部实体遍历)。
...
5. 管理员技能应只对资历4级/信任4级/已登录admin显示和使用，并显式跳过 `@noSkills` 与 PVP 禁用限制。信任3++必须硬性排除。
