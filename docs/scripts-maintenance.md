...
> 调试经验沉淀（2026-08-22）：新增 [Agent 调试经验与常见坑](agent-debug-experience.md)，记录卡久才解决的通用问题（服务器冷启动启动器陷阱、编码、ScriptAgent 编译、H2/Exposed、验证与提交惯例）；新会话 Agent 遇到疑似环境问题时先查该文档。
>
...
### 6. 后续修正（第二轮实测：Wiki 仍留旧菜单 + 多余"返回"）

- **线上表现**：论坛正常了，**Wiki 仍会留一个旧菜单**，并且"在我们按钮堆叠之外"多出一个"返回"按钮。
- **根因（Wiki 侧，与论坛那次不同）**：Wiki 阅读页（MenuV3）里的 `最近修改` 直接调用了旧式菜单
  `openWikiHistoryMenu(player, page.id)`，**没有先 `close()` 当前对话框**。旧式菜单（`MenuBuilder` → `Call.menu`）
  不会顶掉服务端对话框，于是两个菜单同屏：旧菜单自带"返回列表/关闭"，就成了那个"堆叠之外的多余返回"。
  论坛侧此前各入口都写了 `close()`（`格式帮助`/`最近变更`/`管理分区`/`回收站`），所以只有 Wiki 中招。
- **修法**：`wiki.kts` 的 `最近修改` 改为 `close(); openWikiHistoryMenu(...)`（与论坛同款写法）。
- **顺带把 help 侧做统一**（同一根因的另一半）：`runHelpCommand` 现在**一律在独立协程里执行命令**，
  并且帮助菜单的**条目列表**（玩家/投票/管理/其他 指令列表）在点击时**统一先 `MenuNav.push` 返回目标**，
  不再区分 `item.action` 与 `runCommand` 两条分支——之前只有 `runCommand` 分支接了返回目标，
  搜索/指令列表里走 `action` 的条目（例如 `/wiki`）既没有返回目标、又把旧菜单挂住。
...
- **版本对应**：`prerelease-2026.09.13.B495`（提交 `92471dd`）的 `work` 子模块 = Mindustry **v160.3** 提交 `254fd3e`；
  JAR 内 `version.properties` 为 `build=160.3`、`mod.hjson` 为 `minGameVersion="160.3"`。
  MindustryX **正式发行版仍是 X37（v160.1）**，160.3 目前只在预览通道。
- **构建物**：`mdtserver/server-2026.09.13.B495.jar`，SHA-256
  `D2872D30598BFC2557A8122BF1A622F7263AF783B3636E09CFF5BCF2778E4C09`；`server.properties` 的 `jar=` 已切到它
  （切换前的整份配置备份为 `server.properties.bak-x37`）；X37 / B491 / B485 都保留为回滚线。
- **上游改动范围**：B493→B495 只有 3 个提交（世界未加载时收到单位生成包不再崩溃；加载中玩家跳过低优先级包；
  合并 v160.3），**无接口破坏**，插件侧不需要适配。与我们有重叠的是 v160.2/v160.3 的 **Data Patch / 数据资产**修复
  （data patch 音效仅在 >100kb 时走流式；data patch 的 `requiredPlanets`；加入物品后地图崩溃）——
  正是 CP/DP 内容补丁与音乐资产同步依赖的那条链。
- **参考源码**：`参考项目/Mindustry-master` 切到 `tags/v160.3`（`254fd3e`）；
  `参考项目/MindustryX-main` 切到 `92471dd` 并同步子模块（`Arc` v160.2 `68a04fab`、`work` 254fd3e）。
- **验证**：清空 `config/scripts/cache` 后冷启动 = `共找到157脚本,加载成功153,启用成功148,出错0`（`ScriptAgent c2823c1`），
  无异常、6567/6859/10099 正常、命令 Socket `status` 可用、地图正常加载。
- 未覆盖：真实客户端（含安卓端）在新预览版上的观感与内容同步实测。

## 2026-09-13（第六批）：菜单客户端缩放（移动端默认 0.85）

类型：兼容性改进（用户反馈："比例相对固定，对安卓端玩家可能不够友好，可以手动减去 5% 左右的缩放，
或者允许玩家自行调整，或者检查缩放逻辑"）

- **先查缩放逻辑（结论：菜单确实不吃客户端 scl）**：`UiTreeBuilder.applyCellProp` 把服务端下发的数值
  直接写进 `cell.width/pad/height`，原版 UI 的 `Scl.scl()` 只作用于组件内部（字体/样式）；
  `UiKey` 里也没有"缩放后的宽度"这一类键（`scaling` 是 `Image.setScaling`，与 UI 缩放无关）。
  所以同一份 `rootWidth` 在不同 scl 的设备上比例不同：实测窗口 2560×961 的 scl≈1.07，安卓常见 0.45~1.2，
  手机竖屏 720px 宽时 860px 会超过物理宽度。
- **两个可用钩子**：`NetConnection.mobile`（X37 已确认存在，服务端可读）与
  `condition("portrait"|"landscape"|"width>=N")`（客户端求值，`width = 场景宽 / Scl.scl(1f)`）。
- **落地**：`MenuV3` 新增 `uiScale`（默认 `defaultScale(player)`：移动端 `MOBILE_SCALE = 0.85f`、桌面 1.0，
  可用区间 0.6~1.4），统一乘到 `rootWidth`/`optionHeight`/`cellPad`/`pane` 高度/`image` 尺寸；
  页面 DSL 可显式覆盖。桌面观感**不变**（用户已确认现在的比例正常），只收移动端一档，
  窄屏竖屏手机也能塞下（860→731）。整体再调只需改 `MOBILE_SCALE` 一个常量。
- **验证**：X37 冷启动 `共找到157脚本,加载成功153,启用成功148,出错0`（13:34:01），无异常。
- **未做（2026-09-13 用户已定口径后暂停在这里）**：
  ①「允许玩家自行调整」= **菜单内加「界面缩放 −/+」按钮**（用户选定，尚未实现）——需要一个"每玩家偏好"的
  持久化，动手前先定存储口径（新增表 / 文件）；②「按屏幕宽窄下发两套尺寸」（`condition` 可行，
  但两套的 `option()` 回调 id 需要加前缀分别注册）暂不做；③移动端实机效果由**用户用安卓设备实测后反馈**，
  届时按反馈调 `MOBILE_SCALE`（当前 0.85）。
...
Wiki 偏阅读风，帖子增加文字空间并调整按钮位置/大小、两项可占更大面积；超时必须够长以免编辑内容丢失；保留按权限显示的按钮）

### 1. 成就页调整（`wayzer/user/achievement.kts`）

- **去掉滚动**：原实现把列表放进固定高度 `pane`，实测**右侧滚动条占位导致滚动范围内的按钮与外面的按钮不对称**。
  现改为 `renderPaged` 直接整页渲染（每页 6 条）+ 末尾翻页，**不使用任何滚动区域**。
- **超时**：`ACHIEVEMENT_PAGE_TIMEOUT_SECONDS = 300`（原 60 秒太短）。
- **成就管理入口**：可见性从"仅 `wayzer.admin.achievement`"放宽为
  "该权限 **或** 信任系统管理员（`isTrustAdmin`）"。原因：该权限注册在 `@admin` 组，
  而 `hasPermission` 的 `@admin` 只由玩家的**服务器 admin 标志**决定，MDT 的 4级/运维是按信任系统判定的
  （Wiki 那边用的就是 `isTrustAdmin`），所以 4级在菜单里看不到管理入口。

### 2. `MenuV3` 补两个能力（`coreMindustry/lib/menuV3.kt`）

- `label(text, align, wrap = false)`：新增 `wrap` 参数。**长文章必须 `wrap=true`**，否则 Label 按整行宽度撑开布局。
- 会话顶替处理：同一玩家开新菜单时（`hideExisting` 默认 true），`send()` 会回收旧会话并**唤醒其 `await`**，
  避免"每次翻页/跳转都留下一个等到超时的协程"；旧会话不能再调 `close()`（所有会话共用同一 `menuId`，会把当前菜单一起关掉）。

### 3. Wiki 接入（`wayzer/user/wiki.kts`）

- 迁移到 `MenuV3` 的页面：**列表页、页面阅读页、最近修改、格式帮助**。
- 阅读风版式：`fillScreen = true` + 内容限宽 `WIKI_MENU_WIDTH = 620f`；
  **只有正文放进 `pane("wikiBody", 300f)`**（滚动条不再挤压按钮），正文用
  `label(..., align = "left", wrap = true)` 左对齐自动换行；按钮按行分组（分页一行、导航一行、编辑/保护锁一行）。
- 权限按钮照旧：`管理Wiki`（3+）、`编辑此页`（3+ 且未被保护锁限制）、`保护锁`（4级/admin）按条件添加，权限不足不渲染。
- 超时保持 **30 分钟**（`WIKI_MENU_TIMEOUT_MILLIS`），编辑输入同为 30 分钟 —— 满足"不低于 30 分钟"的要求，不会因超时丢编辑。
- 编辑/管理/回收站确认等管理流程仍是聊天菜单；从新菜单进入前先 `close()`（菜单是模态的，会挡住聊天输入框）。

### 4. 帖子系统接入（`wayzer/user/forumPosts.kts`）

- 迁移到 `MenuV3` 的页面：**分区列表、帖子列表、帖子阅读页、评论页**。
- 版式：内容限宽 `FORUM_MENU_WIDTH = 640f`，正文进 `pane("forumBody", 300f)`、评论进 `pane("forumComments", 300f)`；
  按钮按行分组并变小：分页 `column(3)`、点赞/点踩 `column(2)`、评论三连 `column(3)`、
  修改/保护锁 `column(2)`、置顶/锁定/删除 `column(3)`、格式帮助/返回/关闭 `column(3)`。
- 权限按钮全部保留原判定：`发布帖子`（canUseForum）、`修改帖子`（作者或 3+，受保护锁限制）、
  `删帖/置顶/锁定`（3+，删除额外受保护锁限制）、`保护锁`（4级/admin）、`回收站/管理分区`（按角色）——权限不足不渲染。
- 超时保持 **30 分钟**，发帖/评论/编辑输入同为 30 分钟。
- 发帖/改帖/发评论/删帖确认/分区管理/回收站确认仍是聊天流程：进入前先 `close()`。

### 5. 验证与未覆盖边界

- 验证：清空编译缓存、全量重编译（X37）→ **`共找到157脚本,加载成功153,启用成功148,出错0`**，无异常、Socket 正常、端口全释放。
- **未覆盖**：所有观感（限宽、分组、阅读区高度、滚动条、按钮大小）与实际点击流程都必须真实客户端确认。
- 仍是聊天菜单、未迁移的部分：Wiki 编辑/管理/回收站/删除确认；帖子发帖/编辑/评论输入/分区管理/回收站/彻底删除确认。
  这些是管理流程，版式收益小、改动风险大，等页面观感定稿后再决定是否一并迁移。
...
- `wayzer/security/securityGuard.kts`：
  - **恢复**风控模式下被限制游客的聊天与未授权指令丢弃（`isSecurityRestrictedGuest + shouldDropGuestInput`，保留 login/register/help 等允许指令）；`tipGuest` 文案与风控菜单说明恢复为“聊天/部分操作已被限制”口径。
  - **移除“聊天/指令过快增加风控分”**：`hitLimiter`（仅供 OnChat 输入限速使用）超限时的 `registerStrike` 改为 `addScore=false`——仍保留本地降级（第1次警告→第2次踢出→多次IP封禁），但不再调用 `addAnomaly`，聊天/指令刷屏不再推高异常分、不再触发自动进入风控模式；连接异常（多连接/多身份、增强风控未登录连接）仍照常计分。
- 冷启动验证：`共找到157脚本,加载成功153,启用成功148,出错0`（securityGuard 编译与加载成功）。未覆盖边界：风控模式下游客聊天被丢弃、聊天/指令过快不再加分（均需真实客户端触发，逻辑与既有事件路径一致）。
...
- `docs/custom-menu.md`：**Mindustry 160.1 自定义菜单（实验功能）**——服务端下发菜单的 API、会话层设计、
  兼容回退方式、Kotlin 脚本环境踩点，以及 wiki/帖子/商店/成就四个系统的接入说明。
