...
> **2026-09-19 修复**：阅读页的"最近修改"此前会与 Wiki 页面**同屏残留两个菜单**（旧式菜单不会顶掉服务端对话框），
> 导致画面上多出一个无意义的"返回"。现在改为 **先 `close()` 当前菜单再打开旧式菜单**；
> 同时"返回"按钮（从 MDT帮助 进入时出现）会先关掉当前菜单再回上一页。
> 新增页面时请遵守：**MenuV3 → 旧式菜单 要 `close()`；MenuV3 → 上一页 也要先 `close()`**（详见 `custom-menu.md` 的 MenuNav 一节）。
