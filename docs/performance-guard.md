...
`coreUnitRespawnCompat.kts` 覆盖首次加入、内部重同步、核心单位变化和主动取消附身，可靠补发 `PlayerSpawn`，并按 `Unit -> Player` 顺序在 0/1/3/8 秒执行有界定向快照重试。第一轮实体快照使用 B480 自定义服务端 API 可靠发送，后续才使用 UDP 快速重试；不会触发全量 `/sync`。
