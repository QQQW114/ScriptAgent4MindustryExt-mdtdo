...
进入新性能等级时会广播实际采取的重要措施，例如清理火焰/子弹、限制单位、关闭世界处理器和逻辑处理器。等级保持期间若继续发生清理，会按默认 30 秒聚合广播真实数量；PPS、严重上行和 L4 数量前三清理也显示准确单位/炮台数。压力采样与措施循环都有逐轮异常隔离，一次实体或规则同步异常不会让整套系统永久停止。
