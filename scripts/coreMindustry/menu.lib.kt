...
interface IMenuOpenGuard {
    /**
     * @return null 表示允许打开菜单；非空字符串表示拒绝打开，并将该字符串提示给玩家。
     */
    fun menuBlockReason(player: Player, title: String, msg: String, followup: Boolean): String?

    companion object : ServiceRegistry<IMenuOpenGuard>()
}
...
open class MenuBuilder<T : Any>(
    ...
    suspend fun sendTo(player: Player, timeoutMillis: Int = 60_000): T? {
        ...
        IMenuOpenGuard.getOrNull()?.menuBlockReason(player, title, msg, followup)?.let {
