...
private var maxOnlineForAssetSync by config.key(6, "允许发起运行时音乐资产重同步的最大在线人数；超过后仅可播放已同步歌曲")
...
private class MusicResyncWaiter(
...
private val musicResyncWaiters = mutableMapOf<mindustry.net.NetConnection, MusicResyncWaiter>()
...
private fun limitText(): String = buildString {
    ...
    appendLine("[gray]运行时音乐同步人数上限：[gold]${maxOnlineForAssetSync.coerceAtLeast(1)}[gray] 人")
    ...
    appendLine("[white]/music limit syncplayers 6")
}
...
private fun cleanupIdleTransientMusicAssets() {
    ...
    musicResyncWaiters.values.forEach { keepPaths.add(it.track.assetPath) }
...
private suspend fun waitForMusicSyncCapacity(player: Player): String? {
    val online = Groups.player.size()
    val maxOnline = maxOnlineForAssetSync.coerceAtLeast(1)
    if (online > maxOnline) {
        return "当前在线 $online 人，超过运行时音乐同步安全上限 $maxOnline 人"
    }
    ...
    while (System.currentTimeMillis() < deadline) {
        ...
        val currentOnline = Groups.player.size()
        if (currentOnline > maxOnline) return "当前在线 $currentOnline 人，超过运行时音乐同步安全上限 $maxOnline 人"
...
fun isMusicAssetSyncActive(): Boolean = processingMusicAssetPath != null || musicResyncWaiters.isNotEmpty()

private suspend fun sendWorldAndAssetsCompat(player: Player, track: MusicTrack): Boolean {
    val con = player.con ?: return false
    val previous = musicResyncWaiters[con]
    if (previous != null) {
        error("上一次音乐同步仍在等待客户端完成：${previous.track.display}")
    }
    ...
    if (method != null) {
        val waiter = MusicResyncWaiter(
            ready = CompletableDeferred(),
            playerUuid = player.uuid(),
            playerName = player.plainName(),
            track = track,
        )
        musicResyncWaiters[con] = waiter
        try {
            val readyTimeout = assetWorldReadyTimeoutFor(track)
            method.invoke(Vars.netServer, player)
            val confirmed = withTimeoutOrNull(readyTimeout) {
                waiter.ready.await()
                true
            } == true
            val elapsed = System.currentTimeMillis() - waiter.startedAt
            if (!confirmed) {
                error(
                    "等待客户端完成资产/世界同步超时(${elapsed}ms/${readyTimeout}ms, " +
                        "determiningAssets=${con.determiningAssets}, receivingAssets=${con.receivingAssets})"
                )
            }
            logger.info(
                "点歌资产/世界同步已确认: player=${player.plainName()} track=${track.display} " +
                    "size=${formatBytes(track.sizeBytes)} cost=${elapsed}ms"
            )
            delay(postConfirmPlayDelayMillis.coerceAtLeast(0L))
            return true
        } finally {
            if (musicResyncWaiters[con] === waiter) {
                musicResyncWaiters.remove(con)
                waiter.ready.cancel()
            }
        }
    }
...
private fun syncAndPlay(track: MusicTrack, players: List<Player>, operator: String, interrupt: Boolean = true) {
    ...
    targets.forEach { player ->
        ...
        } else if (Groups.player.size() > maxOnlineForAssetSync.coerceAtLeast(1)) {
            clearPendingPlay(uuid, serial)
            player.sendMessage(
                "[yellow]当前在线 ${Groups.player.size()} 人，超过音乐资产同步安全上限 " +
                    "${maxOnlineForAssetSync.coerceAtLeast(1)} 人；为避免上行爆满，本次不会同步新歌曲。"
            )
        } else {
...
private fun musicVoteSummary(vote: MusicVote): String {
    ...
    return "[green]同意/听歌 ${vote.agree.size}/$total[gray]，中立 ${vote.neutral.size}，拒绝 ${vote.disagree.size}，未表态 $notVoted"
}
...
private fun finishMusicVote(voteId: Long) {
    ...
    val result = """
        ...
        |[green]听歌人数：${vote.agree.size}/$total[gray]；中立 ${vote.neutral.size}，拒绝 ${vote.disagree.size}，未表态 $notVoted。
    """.trimMargin()
    onlineMusicPlayers().forEach { it.sendMessage(result) }

    if (vote.starterUuid != null && vote.agree.size * 100 < total * lowAgreeCooldownPercent.coerceAtLeast(0)) {
    ...
    logger.info("点歌投票结束: ${vote.track.display}, agree=${vote.agree.size}, neutral=${vote.neutral.size}, no=${vote.disagree.size}, total=$total")
}
...
command("music", "服务器点歌/音乐播放") {
    ...
    body {
        ...
        when (first) {
            null, "menu", "菜单" -> {
                if (p == null) reply(musicCommandHelp(canAdmin).with())
                else openMusicMenu(p, canAdmin)
            }
            "help", "帮助" -> reply(musicCommandHelp(canAdmin).with())
            "yes", "y", "1", "同意" -> {
            ...
            "limit", "limits", "限制" -> {
                ...
                when (sub) {
                    ...
                    "syncplayers", "syncplayer", "同步人数" -> {
                        val count = value?.toIntOrNull()
                            ?: returnReply("[red]用法：/music limit syncplayers <人数>，20M上行建议不高于6。".with())
                        if (count !in 1..32) returnReply("[red]同步人数上限必须在1到32之间。".with())
                        maxOnlineForAssetSync = count
                        reply("[green]已设置运行时音乐资产同步安全上限为：[gold]$count 人".with())
                    }
            ...
            "cancel", "取消" -> {
                ...
                val input = arg.joinToString(" ").takeIf { it.isNotBlank() }
                    ?: returnReply(musicCommandHelp(canAdmin).with())
                prepareAndThen(p, input, vote = true)
            }
        }
    }
...
private fun musicCommandHelp(canAdmin: Boolean): String = buildString {
    ...
    if (canAdmin) {
        appendLine("[gold]管理：[white]/music limit size|duration|cache|syncplayers <值>[gray] 调整限制")
        appendLine("[gold]管理：[white]/music stopall|cancel[gray] 停止全服音乐/取消点歌投票")
    }
}

listen<EventType.PlayerLeave> { event ->
    val uuid = event.player.uuid()
    event.player.con?.let { con -> musicResyncWaiters.remove(con)?.ready?.cancel() }
...
listen<EventType.PlayerConnectionConfirmed> { event ->
    ...
    val waiter = musicResyncWaiters.remove(con) ?: return@listen
    // 该事件发生在 NetServer.connectConfirm 检查 hasConnected 之前，说明客户端已完成世界加载。
    // 此时恢复为已连接既能抑制重复 motd/PlayerJoin，又能给点歌流程提供真实的客户端就绪信号。
    ...
    if (isMusicAssetSyncActive()) {
        logger.warning(
            "新玩家 ${event.player.plainName()} 在运行时音乐资产同步期间进入；" +
                "原版全局资产表可能让其额外参与当前歌曲协商。active=${musicResyncWaiters.size} " +
...
onDisable {
    ...
    musicResyncWaiters.values.forEach { it.ready.cancel() }
    musicResyncWaiters.clear()
    cleanupIdleTransientMusicAssets()
}
