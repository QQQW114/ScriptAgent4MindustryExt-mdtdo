...
fun Player.showDetail() {
    val scriptInfo = with(funRules) { currentMapScriptStatusText(activeOnly = true) }
        .takeIf { !it.contains("当前没有启用的地图脚本") }
    ...
    }) + customModeIntroduce + listOfNotNull(scriptInfo)).autoPage()
...
fun Player.showInfo() {
    ...
    val msg = buildString {
        ...
        val loadedScripts = with(funRules) { currentMapScriptStatuses(activeOnly = true).size }
        if (loadedScripts > 0) {
            appendLine("[gray]当前启用地图脚本：${loadedScripts}个，可用 /mapinfo 查看。")
        }
    }
