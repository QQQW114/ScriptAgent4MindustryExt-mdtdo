...
private val throttleBaseInterval by config.key(160L, "上行超限同步限制等级1间隔(ms)")
private val throttleStepInterval by config.key(120L, "上行超限同步限制每级增加间隔(ms)")
private val throttleMaxLevel by config.key(3, "上行超限同步限制最高等级")
private val throttleRecoverSamplesRequired by config.key(6, "同步限制退出需要连续恢复采样数")
private val throttleDowngradeSamplesRequired by config.key(3, "同步限制降级需要连续稳定采样数")
private val throttleMinActiveMillis by config.key(45_000L, "同步限制启用后最短保持时间(ms)")
...
private fun updateThrottleLevel(mode: String, trafficAvg: Double, budget: Double): Int {
    if (mode != "experimental" || budget <= 0.0) {
        resetThrottleState()
        return 0
    }
    ...
    var target = when {
        trafficAvg >= budget * trafficCriticalRatio -> 3
        trafficAvg >= budget * trafficLimitRatio -> 2
        trafficAvg >= budget * trafficWarnRatio -> 1
        trafficAvg <= recoverMbps -> (throttleLevelState - 1).coerceAtLeast(0)
        else -> throttleLevelState
    }
...
private fun trafficPressureLevel(mode: String, trafficAvg: Double, budget: Double, throttleLevel: Int): Int {
    if (mode != "experimental" || throttleLevel <= 0 || budget <= 0.0) return 0
    return when {
        trafficAvg >= budget * trafficCriticalRatio -> 3
        trafficAvg >= budget * trafficLimitRatio -> 2
        else -> 1
...
private fun buildReason(tpsLevel: Int, trafficLevel: Int): String = when {
    tpsLevel > 0 && trafficLevel > 0 -> "TPS与上行同时超限"
    tpsLevel > 0 -> "TPS过低"
    trafficLevel > 0 -> "上行需求超预算"
    else -> "正常"
}
...
private fun stablePressureLevel(rawLevel: Int, avgTps: Double): Int {
    ...
    // 退出压力比普通降级更严格：必须达到恢复线并连续稳定，避免 0/1 反复横跳。
    if (rawLevel == 0) {
        resetDowngradeCandidate()
        if (avgTps < tpsThresholds().recover) {
            recoverSamples = 0
            return previous
        }
        recoverSamples++
...
private fun updatePressureSnapshot() {
    ...
    if (mode == "off") {
        snapshot = PressureSnapshot(
            ...
            averageTps = if (tpsSamples.isEmpty()) currentTps().toDouble() else tpsSamples.sum().toDouble() / tpsSamples.size,
            currentTrafficMbps = with(trafficMonitor) { currentTrafficMbps() },
            averageTrafficMbps = with(trafficMonitor) { averageTrafficMbps() },
            trafficBudgetMbps = with(trafficMonitor) { trafficBudgetMbps() },
    ...
    if (tpsSamples.size < sampleSize.coerceAtLeast(1)) {
    ...
    val tpsLevel = tpsPressureLevel(mode, avgTps)
    val throttleLevel = updateThrottleLevel(mode, trafficAvg, budget)
    val trafficLevel = trafficPressureLevel(mode, trafficAvg, budget, throttleLevel)
    val rawLevel = maxOf(tpsLevel, trafficLevel)

    val level = stablePressureLevel(rawLevel, avgTps)
    ...
    snapshot = PressureSnapshot(
        ...
        bullets = Groups.bullet.size(),
        syncEntities = Groups.sync.size(),
        reason = buildReason(tpsLevel, trafficLevel),
    )

    if (snapshot.level > lastBroadcastLevel) {
        broadcast(
            "[yellow][服务器压力] {reason}，压力等级升至 [white]{level}[yellow]；TPS均值 [white]{tps}[yellow]，估算上行 [white]{traffic} Mbps[yellow]。".with(
                "reason" to snapshot.reason,
                "level" to snapshot.level,
                "tps" to snapshot.averageTps.roundToInt(),
                "traffic" to "%.2f".format(snapshot.averageTrafficMbps),
            )
        )
        lastBroadcastLevel = snapshot.level
    } else if (snapshot.level == 0 && lastBroadcastLevel > 0 && recoverSamples >= 2) {
        broadcast("[green][服务器压力] TPS/上行已恢复，退出自动压力等级。".with())
...
fun trafficOverLimit(): Boolean = snapshot.mode == "experimental" && snapshot.throttleLevel > 0
...
fun pressureStatusText(): String {
    ...
    return """
        ...
        |[cyan]压力等级：[white]${s.level}[]（TPS:${s.tpsLevel} / 上行:${s.trafficLevel} / 同步限制:${s.throttleLevel}）
        |[cyan]TPS：[white]${s.currentTps}[] / 均值 [white]${s.averageTps.roundToInt()}
        |[cyan]TPS阈值：[white]${thresholdText()}
        |[cyan]估算上行：[white]${"%.2f".format(s.averageTrafficMbps)} Mbps[] / 预算 [white]${"%.2f".format(s.trafficBudgetMbps)} Mbps
