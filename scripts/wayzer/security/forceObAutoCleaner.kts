...
private val sameIpProtectAdmins by config.key(true, "清理同IP多账号时是否保护3++协管/4级/admin玩家")
...
private fun onlineForceObCandidates(): List<Candidate> =
    Groups.player.toList()
        .mapNotNull { player ->
            ...
            if (with(trustLevel) { hasTrustLevel(player, "3++") }) return@mapNotNull null
...
private fun onlineSameIpDuplicateCandidates(excludeUuids: Set<String> = emptySet()): List<SameIpCandidate> {
    ...
    val identities = Groups.player.toList().mapNotNull { player ->
        ...
        OnlineIdentity(
            ...
            // 3++/4 都是人工任命的工作人员，高人数同IP清理不应自动踢出。
            admin = with(trustLevel) { hasTrustLevel(player, "3++") },
...
private fun statusText(): String {
    ...
    return """
        ...
        |[gray]边界：强制观战清理只踢出已登录且处于 /vote ob 或 /forceOB 记录中的普通玩家；同IP清理只在超过阈值时处理已登录的同IP多账号，默认保护3++协管、4级/admin。
    """.trimMargin()
}
...
command("forceobclean", "管理指令：高人数时清理强制观战玩家") {
    ...
    body {
        ...
        if (player?.let { with(trustLevel) { isPluginAdmin(it) } } == true && op in setOf("on", "enable", "开启", "off", "disable", "关闭")) {
            returnReply("[red]3++协管只能查看状态或执行单次清理，不能修改自动清理开关。".with())
        }
        when (op) {
